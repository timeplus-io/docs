...
时间plus是一个流式分析平台和数据流中的生命值。 Timeplus `streams` are similar to `tables` in the traditional SQL databases. 两者基本上都是数据集。 关键的区别是，时间加载流是一个只附加的、未受欢迎的、不断变化的事件组。
...
您也可以在 Timeplus 中创建 **个外部流** 来查询外部系统中的数据，而不要将数据加载到 Timeplus 中。 这样做的主要好处是在外部系统(例如Kafka)中保持一个单一的真相来源，而不重复这些来源。 您可以以与其他流相似的方式运行流媒体分析，但有一些限制。
...
要查询外部系统中的数据，请以类似的方式运行流媒体SQL，比如： `SELECT count(*) FROM my_external_stream` 你也可以根据外部流创建 [视图](view) 或 [实际化视图](view#materialized-view)
...
3. 由于原始数据没有保存在Timeplus中，我们不能为每个事件在摄取时间附加事件时间或索引时间。 您可以在查询中指定表达式的事件时间，例如 `肿瘤(ext_stream,to_time(raw:order_time),1m)`
