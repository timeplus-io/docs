...
流指数据在哪里运行，每个数据包含一个事件时间。 时间plus将此属性作为事件的一个重要特征。
...
* 在执行基于时间窗口的聚合时， 例如 [肿瘤](functions#tumble) 或 [钩子](functions#hop) 以获取每次窗口中的下载数据或外部数据， 时间plus将使用事件时间来决定某些事件是否属于特定窗口
...
当你 [摄取最大的数据](ingestion) 到 Timeplus 时，你可以在数据中指定一个属性来最能代表事件时间。 即使该属性是在 `字符串` 类型中，时间加载将自动转换为时间戳以便进一步处理。

如果您不在向导中选择属性，则Timeplus将使用摄取时间来显示事件时间。 当时间plus接收数据时。 这可能对大多数静态或维数据很有用，例如带有邮政编码的城市名称。

### 在查询时指定

[肿瘤](functions#tumble) 或 [连接](functions#hop) 窗口函数将可选参数作为事件时间列。 默认情况下，我们将使用每个数据中的事件时间。 然而，您也可以指定一个不同的列作为事件时间。
