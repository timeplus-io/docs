...
* 流引擎
  ...
  * 我们简化了 [session](/functions_for_streaming#session) 时间窗口：如果您想要创建子流， 您不再需要设置 `keyBy` 列作为会话窗口的一个参数。 只需使用 `SELECT... FROM session(..) PARTITION BY keyBy` . Other time window functions([tumble](/functions_for_streaming#tumble) and [hop](/functions_for_streaming#hop)) support the `PARTITION BY` in the same way.
