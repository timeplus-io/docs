...
* 流引擎
  * 添加了一个 [圆](functions#round) 函数来将一个值转到指定的小数位位置。
  * 改进集群支持。
* 源、 汇、 API 和 SDK
...
我们已将乙型六氯环己烷客户迁移到乙型六氯环己烷。 https://demo.timeplus.com不再可用。 如果您有测试测试帐户，请访问 https://beta.timeplus.cloud/demo。
...
* 界面改进
  ...
  * 增强SQL 编辑器以支持常见查询片断，例如 `通过window_start、window_end`
