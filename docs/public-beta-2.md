...
**Enhancements: Data Ingestion **
  * We've made it easier for you to push data via Ingest REST API. We've added a wizard page in our console to guide you through the steps of selecting a target stream, a target template, an API key, and then generating a sample code for the push request. [Learn more.](ingest-api)
