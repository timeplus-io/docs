...
Configurable via runtime settings or YAML configuration.
...
Example:
```sql
INSERT INTO device_metrics SELECT * , _tp_sn FROM table(aggr_metrics) settings max_insert_block_bytes=4096000, max_insert_threads=8;
```
...
Configurable via runtime settings only.
