...
3. The Remote UDF in Timeplus is not designed for aggregation — remote functions are always scalar, so they never receive the [`_tp_delta`](/changelog-stream) column that changelog input adds to an aggregate function. Please turn to [JavaScript](/js-udf#udaf) or [Python](/py-udf#udaf) local UDFs for User-Defined Aggregate Functions (UDAF).
