...
*Timeplus Core Engine (Proton v1.5.7):*
  * You can now join multiple [versioned streams](versioned-stream) using `LEFT JOIN` and by assigning primary key(s). Results will be emitted whenever there are updates to either side of the JOIN. [Learn more about JOINs](joins)
...
*Timeplus Cloud and Timeplus Enterprise:*
  ...
  * In the SQL editor, click the stream name to show the stream's schema.
