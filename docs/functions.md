...
For example `path_full('https://docs.timeplus.com/functions#if')` will return the string `/functions#if`

### fragment

`fragment(url)` Extracts the fragment from a URL. If there is no fragment, return an empty string.

For example `fragment('https://docs.timeplus.com/functions#if')` will return the string `if`
